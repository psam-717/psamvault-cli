...
|---|---|---|---|---|
| [PLAN_agent_safe_vault.md](active/PLAN_agent_safe_vault.md) | Reveal guardrail (caller classification, policy, TTY-only approval tokens, audit log) + credential-blind ingress (claim codes) + use-side policy/leases | agent security | **wave 1 (the reveal guardrail) merged in PR #54, shipped in v0.6.0**; **wave 2 slimmed on review 2026-09-25 — the claim handoff only, open as PR #63** | Wave 2 = the claim handoff for all three families (`add`/`ak-add`/`note-add` claim codes, `psamvault pending` list/cancel) — one CLI PR, docs in the same PR. Wave 3 = `--wait`/`--timeout`, the file-migration flags (`--from-file`/`--from-key`/`--from-env`), the loopback form + use-side policy (fail-open default). Independent: backend `/vault/proxy` → `410 Gone` |

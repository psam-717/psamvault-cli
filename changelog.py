...
CHANGELOG: dict[str, list[str]] = {
    "0.6.0": [
        ...
        "       passphrase, as a server-side slot plus a portable kit file you keep off-device",
        "New:   `restore` regains vault access on a new or wiped machine with no session at all",
        "New:   signup offers to set up a backup; uninstall warns when deleting the account would remove the",
        "       last copy of the vault key",
        "New:   Reveal guardrail — `get`, `ak-get`, `note-get`, `export --plaintext` and `--copy` refuse when the",
        "       caller is an agent, printing the capability alternatives instead of the secret",
        "New:   `approve <entry> --for-agent [--ttl N]` hands an agent ONE reveal of ONE entry from your own",
        "       terminal — single use, 15s-1h, dropped on logout; a whole-vault dump can never be approved",
        "New:   `~/.psamvault/policy.json` chooses who may reveal a secret — human-only (default), strict or",
        "       open — plus a per-entry allowlist; `~/.psamvault/audit.jsonl` records every reveal decision",
        "       and never a secret",
        "New:   a global `--agent` flag and `PSAMVAULT_AGENT` for hosts that set no marker of their own",
        "New:   HTTP 429 raises a typed rate-limit error that says wait, not retry",
        "Fixed: `restore` now really refuses a revoked kit — a broad except was swallowing the refusal",
        "Fixed: `backup verify --kit` names the kit's real server-side slot instead of a sliced placeholder",
        "Fixed: upgrade snapshots can no longer collide on Windows' ~15 ms clock and silently merge",
        "Fixed: a CI job is no longer graded as an agent — `CI=true` is recorded as unattended; an agent in CI",
        "       is still refused by its own markers or ancestry, and policy `strict` still refuses CI",
        "Changed: the guardrail decides at the emit point (after fetch and decrypt) — a failed lookup never",
        "       burns an approval",
        "Changed: `export --plaintext` names how many secrets it will expose before asking; `logout` drops",
        "       every pending approval",
        "Changed: `upgrade` (pipx track) reinstalls the pinned version from PyPI, healing URL/TestPyPI/editable",
        "       install sources",
        "Changed: logging in on a new machine explains the device-key mismatch and points at `restore`",
        "Tests: 98 guardrail tests — the caller ladder, the ancestry rules, the policy matrix, the audit",
        "       schema, every allow/deny path and the approval lifecycle — plus 20 backup, 11 restore and",
        "       crypto kit-coverage tests; the four gated commands are driven through the real CLI",
        "Docs:  README sections for backup & recovery and for the agent reveal guardrail, each with a",
        "       when-to-use trigger table; SECURITY covers key material, kit contents and what the",
        "       guardrail does NOT stop (measured limits, not promises)",
    ],
    "0.5.6": [
